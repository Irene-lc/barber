...
public class PagoController {
    ...
    public ResponseEntity<PagoResponse> buscarPorId(@PathVariable String id) {
        try {
            ...
            log.error("Error al buscar pago con id {}", id, e);
            return ResponseEntity.notFound().build();
        }
    }

    @PostMapping
    public ResponseEntity<Void> guardar(@RequestBody PagoRequest dto) {
        try {
            service.guardar(dto);
    ...
    @PutMapping("/{id}")
    public ResponseEntity<Void> actualizar(@PathVariable String id, @RequestBody PagoRequest dto) {
        try {
